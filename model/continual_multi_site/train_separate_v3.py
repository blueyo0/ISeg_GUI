#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2020/10/26 4:37 下午
# @Author  : Jingyang.Zhang
'''
根据每个中心的数据单独进行训练; 注意此v3版本：优化器初始化在 single_train_round 内部；，
                                        模型仅进行一次初始化，然后进行载入上次最优的模型
'''
...
def single_train_round(site:str, model:torch.nn.Module, loader_train:DataLoader, loaders_val:dict,
                       train_params:dict, snap_path):
    '''
...
def continual_train_rounds(loaders_train: dict, loaders_val: dict, common_params:dict,
    ...
    model = Unet(net_params).cuda()  # 模型初始化一次，后续进行finetuning
    ...
    for site_index, site in enumerate(sites):
        ...
        if site_index != 0:
            ...
            logging.info(f'Load model from ' + os.path.join(os.path.join(common_params['exp_root_dir'],
                                                                       sites[site_index - 1]), 'best_model.pth'))

        # 在当前中心训练模型
        model = single_train_round(site, model=model, loader_train=loaders_train[site], loaders_val=loaders_val,
