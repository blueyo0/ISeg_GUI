#!/usr/bin/python3
# -*- coding: utf-8 -*-
# @Time    : 2020/10/20 8:57 下午
# @Author  : Jingyang.Zhang
'''
根据每个中心的数据进行fine-tuning，代表最大的灾难性遗忘
'''
...
def single_train_round(site:str, optimizer, model:torch.nn.Module, loader_train:DataLoader, loaders_val:dict,
                       train_params:dict, writer, snap_path):
    '''
    ...
    while global_step < train_params['iterations']:
        ...
        if global_step % train_params['print_freq'] == 0:
            logging.info('Iteration %d (lr-%.4f): loss_bce : %.4f; loss_dice: %.4f; loss: %.4f; ' %
                         (global_step, lr_, loss_bce.item(), loss_dice.item(), loss.item()))
...
def continual_train_rounds(loaders_train: dict, loaders_val: dict, common_params:dict,
    ...
    model = Unet(net_params).cuda()  # 定义模型结构
    for site_index, site in enumerate(sites):
        # model = Unet(net_params).cuda()  # 定义模型结构   ######## train_finetuning_separate 修改内容
        logging.info(f'============= Train on {site}')

        # 配置summarywriter
        exp_dir = os.path.join(common_params['exp_root_dir'], site)
        shutil.rmtree(exp_dir, ignore_errors=True)
        os.makedirs(exp_dir)
        writer = SummaryWriter(log_dir=exp_dir)

        # 配置优化器，对于每个中心的数据都重新初始化
        ...
        # 配置模型，选择是否初始化 ========= 区别于 finetuning_bestcurmodel 和 finetuning_onlinemodel
        # if site_index != 0:
        #     model.load_state_dict(torch.load(os.path.join(os.path.join(common_params['exp_root_dir'],
        #                                                                sites[site_index - 1]), 'best_model.pth')))
        #     logging.info(f'Loading model from ' + os.path.join(os.path.join(common_params['exp_root_dir'],
        #                                                                sites[site_index - 1]), 'best_model.pth'))
        # else:
        #     logging.info('Init model.')

        # 在当前中心训练模型
        model = single_train_round(site, optimizer, model, loader_train=loaders_train[site],
                                   loaders_val=loaders_val, train_params=train_params, writer=writer,
                                   snap_path=exp_dir)
...
def main():
    ...
    # 配置多中心训练测试数据
    sites = ['ISBI','ISBI_1.5', 'I2CVB', 'UCL', 'BIDMC', 'HK']  ### fewsites
